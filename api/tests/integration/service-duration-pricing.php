...
$root->exec("INSERT INTO clinics(id,name)VALUES(1,'Synthetic clinic');INSERT INTO users(id,clinic_id,email,display_name,user_type,status)VALUES(1,1,'admin@example.test','Admin','staff','active')");
...
$base=['name'=>'Massage','description'=>'Test only','duration_options'=>[['minutes'=>60,'price_cents'=>11000],['minutes'=>90,'price_cents'=>15500],['minutes'=>120,'price_cents'=>20000]],'lead_time_minutes'=>0,'booking_horizon_days'=>365,'buffer_before_minutes'=>0,'buffer_after_minutes'=>0,'requires_room'=>false,'recurrence_allowed'=>false];
$id=$service->createService($actor,$base,'duration-test')['id'];$rows=$service->services($actor);
if(count($rows)!==1||$rows[0]['duration_options']!==$base['duration_options']||(int)$rows[0]['price_cents']!==11000)throw new RuntimeException('Created duration prices were not returned accurately');
$updated=array_replace($base,['duration_options'=>[['minutes'=>60,'price_cents'=>11500],['minutes'=>120,'price_cents'=>20500]],'active'=>true]);
$service->updateService($actor,$id,$updated,'duration-test');$rows=$service->services($actor);
if($rows[0]['duration_options']!==$updated['duration_options']||(int)$rows[0]['price_cents']!==11500)throw new RuntimeException('Updated duration prices were not returned accurately');
...
echo "5 service duration database checks passed on ".$root->getAttribute(PDO::ATTR_SERVER_VERSION)."; scratch database: $name\n";
