...
$base=['category_id'=>10,'name'=>'Massage','description'=>'Test only','duration_options'=>[['minutes'=>60,'price_cents'=>11000],['minutes'=>90,'price_cents'=>15500],['minutes'=>120,'price_cents'=>20000]],'lead_time_minutes'=>0,'booking_horizon_days'=>365,'buffer_before_minutes'=>0,'buffer_after_minutes'=>0,'requires_room'=>false,'recurrence_allowed'=>false];
$id=$service->createService($actor,$base,'duration-test')['id'];$rows=$service->services($actor);
if(count($rows)!==1||$rows[0]['duration_options']!==$base['duration_options']||(int)$rows[0]['price_cents']!==11000||(int)$rows[0]['category_id']!==10||$rows[0]['category_name']!=='Massage Therapy')throw new RuntimeException('Created service category or duration prices were not returned accurately');
...
echo "8 service category and duration database checks passed on ".$root->getAttribute(PDO::ATTR_SERVER_VERSION)."; scratch database: $name\n";
