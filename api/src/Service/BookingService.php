...
final class BookingService
{
    ...
    public function create(AuthContext $actor,array $body,string $correlationId): array
    {
        ...
        try{$startsAt=new DateTimeImmutable((string)$body['starts_at']);}catch(Throwable){throw new ApiException(422,'validation_error','starts_at must be a valid ISO-8601 timestamp.',['starts_at'=>'Invalid timestamp']);}
        $startsUtc=$startsAt->setTimezone(new DateTimeZone('UTC'));$pdo=$this->database->connection();
        try{
            $pdo->beginTransaction();
            $existing=$pdo->prepare('SELECT * FROM appointments WHERE clinic_id=:clinic AND idempotency_key=:key FOR UPDATE');$existing->execute(['clinic'=>$actor->clinicId,'key'=>$body['idempotency_key']]);if($row=$existing->fetch()){$pdo->commit();return $row;}
            ...
            $insert->execute(['clinic'=>$actor->clinicId,'location'=>(int)$body['location_id'],'client'=>$clientId,'p'=>(int)$body['practitioner_id'],'service'=>(int)$body['service_id'],'duration'=>(int)$body['duration_option_id'],'room'=>isset($body['room_id'])?(int)$body['room_id']:null,'starts'=>$startsUtc->format('Y-m-d H:i:s'),'ends'=>$endsUtc->format('Y-m-d H:i:s'),'buffer_start'=>$params['start'],'buffer_end'=>$params['end'],'source'=>$actor->userType==='client'?'public':($body['source']??'admin'),'key'=>$body['idempotency_key'],'creator'=>$actor->userId]);
