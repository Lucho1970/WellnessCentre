...
final class AdminService
{
    ...
    {
        $this->superAdmin($actor);$this->validateService($body);$pdo=$this->database->connection();
    ...
    public function services(AuthContext $actor): array{$this->superAdmin($actor);$s=$this->database->connection()->prepare('SELECT s.id,s.name,s.description,s.preparation_instructions,s.price_cents,s.lead_time_minutes,s.booking_horizon_days,s.buffer_before_minutes,s.buffer_after_minutes,s.requires_room,s.recurrence_allowed,s.active,d.duration_minutes FROM services s JOIN service_duration_options d ON d.service_id=s.id AND d.active=1 WHERE s.clinic_id=:clinic ORDER BY s.name,d.duration_minutes');$s->execute(['clinic'=>$actor->clinicId]);$result=[];foreach($s->fetchAll() as $row){$id=(int)$row['id'];$minutes=(int)$row['duration_minutes'];if(!isset($result[$id])){$row['durations']=[];unset($row['duration_minutes']);$result[$id]=$row;}$result[$id]['durations'][]=$minutes;}return array_values($result);}
    public function updateService(AuthContext $actor,int $id,array $body,string $correlationId): array{$this->superAdmin($actor);$this->validateService($body);$pdo=$this->database->connection();try{$pdo->beginTransaction();$s=$pdo->prepare('UPDATE services SET name=:name,description=:description,preparation_instructions=:instructions,price_cents=:price,duration_mode=:mode,lead_time_minutes=:lead,booking_horizon_days=:horizon,buffer_before_minutes=:before,buffer_after_minutes=:after,requires_room=:room,recurrence_allowed=:recurrence,active=:active WHERE id=:id AND clinic_id=:clinic');$s->execute(['name'=>trim((string)$body['name']),'description'=>$this->optional($body,'description'),'instructions'=>$this->optional($body,'preparation_instructions'),'price'=>(int)$body['price_cents'],'mode'=>count($body['durations'])>1?'selectable':'fixed','lead'=>(int)($body['lead_time_minutes']??0),'horizon'=>(int)($body['booking_horizon_days']??365),'before'=>(int)($body['buffer_before_minutes']??0),'after'=>(int)($body['buffer_after_minutes']??0),'room'=>(bool)($body['requires_room']??true)?1:0,'recurrence'=>(bool)($body['recurrence_allowed']??false)?1:0,'active'=>(bool)($body['active']??true)?1:0,'id'=>$id,'clinic'=>$actor->clinicId]);$check=$pdo->prepare('SELECT 1 FROM services WHERE id=:id AND clinic_id=:clinic');$check->execute(['id'=>$id,'clinic'=>$actor->clinicId]);if(!$check->fetchColumn())throw new ApiException(404,'service_not_found','Service not found.');$pdo->prepare('UPDATE service_duration_options SET active=0 WHERE service_id=:id')->execute(['id'=>$id]);$d=$pdo->prepare('INSERT INTO service_duration_options(service_id,duration_minutes) VALUES(:id,:minutes) ON DUPLICATE KEY UPDATE active=1');foreach(array_unique(array_map('intval',$body['durations'])) as $minutes)$d->execute(['id'=>$id,'minutes'=>$minutes]);$this->audit->write($actor->clinicId,$actor,$correlationId,'service.update','service',$id);$pdo->commit();return ['id'=>$id];}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}}
    ...
    private function validateService(array $body): void{$this->required($body,['name','price_cents','durations']);if(strlen(trim((string)$body['name']))>150||(int)$body['price_cents']<0)throw new ApiException(422,'validation_error','Enter a valid service name and price.');if(!is_array($body['durations'])||$body['durations']===[])throw new ApiException(422,'validation_error','At least one duration is required.');foreach($body['durations'] as $d)if((int)$d<=0||(int)$d>480||(int)$d%15!==0)throw new ApiException(422,'validation_error','Durations must be 15-minute increments up to 480 minutes.');foreach(['buffer_before_minutes','buffer_after_minutes'] as $f)if((int)($body[$f]??0)<0||(int)($body[$f]??0)%15!==0)throw new ApiException(422,'validation_error','Buffers must be non-negative 15-minute increments.');}
