...
export function ClientApp({ initialError = "" }: { initialError?: string }) {
  ...
  const [account] = useState(() =>
    customerConfigured ? customerInstance.getActiveAccount() : null,
  );
  ...
  useEffect(() => {
    const end = () => {
      ...
      setSession(null);
      setChecking(false);
      setError(t("Your client session has ended. Please sign in again."));
    };
    window.addEventListener("customer-session-ended", end);
    const expires = session
      ? Math.min(session.idle_expires_at, session.absolute_expires_at) * 1000
      : null;
    const timer = expires
      ? window.setTimeout(end, Math.max(0, expires - Date.now()))
      : undefined;
    ...
    const activity = (event: Event) => {
      ...
      void customerFetch("/auth/activity", { method: "POST", body: "{}" })
        .then((data) => {
          updateSessionTimes(data.session);
          setSession(data.session);
        })
        .catch(() => {
          /* Expiry is still enforced by the server and timer. */
        });
    };
    ...
    return () => {
      ...
      window.removeEventListener("customer-session-ended", end);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      setChecking(false);
      setError(
        t("Customer verification timed out. Please retry or sign in again."),
      );
    }, 20000);
    void (async () => {
      try {
        const data = await customerFetch("/auth/me", {
          signal: controller.signal,
        });
        if (
          data?.authenticated !== true ||
          data.authentication_context !== "customer" ||
          !["not_linked", "pending_review", "linked"].includes(
        ...
        if (
          ...
          Math.min(
            ...
            data.session.absolute_expires_at,
          ) *
            1000 <=
            Date.now()
        )
          throw new Error(
            t("Your client session has ended. Please sign in again."),
          );
        if (!controller.signal.aborted) {
          setVerified(true);
          setStatus(data.session ? data : null);
        ...
        if (!controller.signal.aborted)
          setError(
            cause instanceof Error
              ? cause.message
              : t("Unable to verify customer sign-in."),
          );
      } finally {
        ...
        >
          ...
          {!customerConfigured ? (
            ...
            <Typography role="status" sx={{ my: 2 }}>
              {t("Verifying customer sign-in…")}
            </Typography>
          ) : verified ? (
            <Alert severity="success" sx={{ my: 2 }}>
              {t("Customer sign-in verified.")}
            </Alert>
          ) : (
            <Typography sx={{ my: 2 }}>
              {t(
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} mt={3}>
            {customerConfigured && !verified && (
              <Button
                variant="contained"
                disabled={busy || checking}
                onClick={() => void run(customerSignIn)}
              >
                {t(
                  account
                    ? "Sign in again"
                    : "Sign in or create client account",
                )}
              </Button>
            )}
            {account && error && (
