...
export function CustomerWorkspace({ status, onRefresh }: { status: CustomerStatus; onRefresh: () => void }) {
  ...
  return <Stack spacing={2} mt={3}>
    ...
    {status.onboarding_status === 'linked' ? <Stack direction="row" spacing={1}><Button disabled={saving} onClick={() => setMode('profile')}>{t('My profile')}</Button><Button disabled={saving} onClick={() => setMode('appointments')}>{t('My appointments')}</Button></Stack>
