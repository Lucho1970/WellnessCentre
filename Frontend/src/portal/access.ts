...
export type PortalPage = 'dashboard' | 'appointments' | 'clients' | 'calendar' | 'schedule_calendar' | 'business' | 'practitioners' | 'staff' | 'team' | 'widgets' | 'notifications' | 'locations' | 'rooms' | 'services' | 'profile';
...
export function pagesFor(roles: string[], workspace: Workspace): PortalPage[] {
  ...
  if (workspace === 'practitioner') return ['dashboard', 'schedule_calendar', 'appointments', 'calendar', 'profile'];
...
const slugs: Record<PortalPage, string> = { dashboard: '', appointments: 'appointments', clients: 'clients', calendar: 'availability', schedule_calendar: 'calendar', business: 'settings', practitioners: 'practitioners', staff: 'users', team: 'team', widgets: 'dashboard-widgets', notifications: 'notifications', locations: 'locations', rooms: 'rooms', services: 'services', profile: 'profile' };
