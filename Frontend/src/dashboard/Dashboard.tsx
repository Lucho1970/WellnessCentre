...
type Definition = { id: string; renderer: 'metric'|'next_appointment'; title: { en:string; fr:string }; description: { en:string; fr:string }; icon: 'calendar-check'|'clock'|'map-pin'; destination: { page:'appointments' }; sizes:WidgetSize[] };
type Summary = { workspace: Workspace; timezone: string; as_of: string; definitions: Definition[]; values: Record<string,number|NextAppointment|null> };
type Preferences = { version: 1; workspace: Workspace; widgets: Preference[] };
const icons={ 'calendar-check':CalendarCheck,clock:Clock3,'map-pin':MapPin } as const;
    ...
    {ordered.length===0?<Alert severity="info">{t('There are no dashboard widgets available for your current role yet.')}</Alert>:<Box sx={{display:'grid',gridTemplateColumns:'repeat(12, minmax(0, 1fr))',gap:2}}>
      {ordered.map((item,index)=>{
        ...
        return <Card key={item.id} variant="outlined" sx={{gridColumn:{xs:'span 12',sm:`span ${Math.max(span,6)}`,lg:`span ${span}`},opacity:item.enabled?1:.55,minHeight:190,display:'flex',flexDirection:'column'}}>
          ...
          :<CardActionArea component={Link} to={destination} sx={{height:'100%',display:'flex',alignItems:'stretch'}}><CardContent sx={{width:'100%'}}><Stack direction="row" justifyContent="space-between" alignItems="flex-start"><Box><Typography variant="overline" color="primary.main" fontWeight={800}>{title}</Typography>{content}</Box><Icon size={28}/></Stack><Typography color="text.secondary" mt={1}>{description}</Typography><Typography color="primary.main" fontWeight={700} mt={2}>{t('View appointments')} →</Typography></CardContent></CardActionArea>}
        </Card>;
      })}
    </Box>}
