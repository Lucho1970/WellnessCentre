  ...
  return <Stack spacing={3}>
    <Paper component="form" onSubmit={submit} variant="outlined" sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={12}><TextField fullWidth multiline label={t('Description')} value={form.description} onChange={event => field('description', event.target.value)} /></Grid>
        ...
        {form.duration_options.map((option, index) => <Grid size={12} key={option.key}><Stack direction="row" spacing={2} alignItems="center">
          <TextField required fullWidth type="number" label={t('Duration {{number}} (minutes)', { number: index + 1 })} value={option.minutes} onChange={event => changeDuration(option.key, 'minutes', event.target.value)} inputProps={{ min: 15, max: 480, step: 15 }} />
          <TextField required fullWidth type="number" label={t('Price {{number}} (CAD)', { number: index + 1 })} value={option.price} onChange={event => changeDuration(option.key, 'price', event.target.value)} inputProps={{ min: 0, step: .01 }} />
          <IconButton aria-label={t('Remove duration {{number}}', { number: index + 1 })} disabled={form.duration_options.length === 1} onClick={() => removeDuration(option.key)}><Trash2 size={18} /></IconButton>
        </Stack></Grid>)}
        ...
        {(Object.keys(fieldLabels) as (keyof typeof fieldLabels)[]).map(key => <Grid size={{ xs: 6, md: 3 }} key={key}><TextField required fullWidth type="number" label={t(fieldLabels[key])} value={form[key]} onChange={event => field(key, event.target.value)} /></Grid>)}
        <Grid size={12}><TextField fullWidth multiline label={t('Preparation instructions')} value={form.preparation_instructions} onChange={event => field('preparation_instructions', event.target.value)} /></Grid>
        <Grid size={12}><Stack direction={{ xs: 'column', sm: 'row' }}><FormControlLabel control={<Switch checked={form.requires_room} onChange={event => field('requires_room', event.target.checked)} />} label={t('Requires a room')} /><FormControlLabel control={<Switch checked={form.recurrence_allowed} onChange={event => field('recurrence_allowed', event.target.checked)} />} label={t('Recurring bookings')} />{editing && <FormControlLabel control={<Switch checked={form.active} onChange={event => field('active', event.target.checked)} />} label={t('Active')} />}</Stack></Grid>
      </Grid>
      {error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}{saved && <Alert severity="success" sx={{ mt: 2 }}>{saved}</Alert>}
      <Button type="submit" variant="contained" disabled={busy} startIcon={editing ? <Save size={17} /> : <Plus size={17} />} sx={{ mt: 2 }}>{busy ? t('Saving…') : t(editing ? 'Save changes' : 'Add service')}</Button>
    </Paper>
    <Paper variant="outlined" sx={{ p: 3 }}><Typography variant="h5" mb={2}>{t('Services')}</Typography><Stack spacing={1}>{items.map(service => <Stack key={service.id} direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
      <span><Typography fontWeight={700}>{service.name} · {t('from {{price}}', { price: money(Math.min(...options(service).map(option => option.price_cents))) })}</Typography><Typography variant="body2" color="text.secondary">{options(service).map(option => t('{{minutes}} min — {{price}}', { minutes: option.minutes, price: money(option.price_cents) })).join(' · ')} · {t(Boolean(Number(service.active)) ? 'Active' : 'Inactive')} · {t(Boolean(Number(service.requires_room)) ? 'Room required' : 'No room required')}</Typography></span>
      <Button startIcon={<Pencil size={16} />} onClick={() => edit(service)}>{t('Edit')}</Button>
    </Stack>)}</Stack></Paper>
  </Stack>;
}
