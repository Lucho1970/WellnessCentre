...
export function ClientLoginLink() {
  ...
  const request = () => {
    ...
    frame.current?.contentWindow?.postMessage(
      { type: "wellness:account-request", nonce: nonce.current },
      portalUrl.origin,
    );
  };
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      if (
        event.origin !== portalUrl.origin ||
        event.source !== frame.current?.contentWindow
      )
        return;
      if (event.data?.type === "wellness:account-ready") {
        request();
        return;
      }
      if (
        event.data?.type !== "wellness:account-display" ||
  ...
  return (
    <>
      <iframe
        ref={frame}
        src={portalLink("client/session")}
        title={t("Client account status")}
        hidden
        onLoad={request}
      />
      {initials ? (
        <IconButton
          href={portalLink("client")}
          aria-label={t("Open client account")}
        >
        ...
        <Button href={portalLink("client")} variant="contained">
          {t("SignIn")}
        </Button>
      )}
    </>
