...
type Practitioner = { slug: string; display_name: string; public_title: string; public_title_fr: string | null; summary: string | null; summary_fr: string | null; booking_practitioner_id: number };
type Service = {
...
export function ServiceDetails() {
  ...
  return <Container maxWidth="lg" sx={{ py: { xs: 4, md: 7 } }}>
    ...
    <Grid container spacing={5} mt={1}><Grid size={{ xs: 12, md: 8 }}><Stack spacing={3}>
      ...
      <Box><Typography variant="h5" component="h2">{t('Practitioners')}</Typography>{service.practitioners?.length ? <Grid container spacing={2} mt={0}>{service.practitioners.map(person => <Grid key={person.slug} size={{ xs: 12, sm: 6 }}><Card variant="outlined"><CardContent><Typography variant="h6">{person.display_name}</Typography><Typography color="primary">{localized(person.public_title,person.public_title_fr,language)}</Typography><Typography color="text.secondary" mt={1}>{localized(person.summary,person.summary_fr,language)}</Typography></CardContent><CardActions><Button component={Link} to={`/book?service=${encodeURIComponent(service.slug)}&practitioner_id=${person.booking_practitioner_id}`}>{t('Book with {{name}}',{name:person.display_name})}</Button></CardActions></Card></Grid>)}</Grid> : <Typography color="text.secondary" mt={1}>{t('Contact the clinic for practitioner availability.')}</Typography>}</Box>
