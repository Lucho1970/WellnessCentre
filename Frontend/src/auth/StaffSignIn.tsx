...
export function StaffSignIn({ children }: { children: ReactNode }) {
  const { account, configured, isAuthenticated, signIn, signOut, getAccessToken } = useStaffAuth();
  const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const [role, setRole] = useState('');
  useEffect(() => {
    if (!isAuthenticated) { setRole(''); return; }
    let current = true; setBusy(true); setError('');
    getAccessToken().then(token => fetch(`${import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:8080/api/v1'}/auth/me`, { headers: { Authorization: `Bearer ${token}` } })).then(async response => { const body = await response.json(); if (!response.ok) throw new Error(body?.error?.message ?? 'Staff authorization failed.'); if (current) setRole(body.data.roles[0]); }).catch(cause => { if (current) setError(cause instanceof Error ? cause.message : 'Staff authorization failed.'); }).finally(() => { if (current) setBusy(false); });
  ...
  return <Box><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} mb={2} spacing={1}><Typography variant="body2" color="text.secondary">Signed in as {account?.name ?? account?.username}{role ? ` · ${role}` : ''}</Typography><Button size="small" startIcon={<LogOut size={16}/>} onClick={() => run(signOut)} disabled={busy}>Sign out</Button></Stack>{busy && <Stack alignItems="center" py={4}><CircularProgress/><Typography mt={2}>Verifying staff access…</Typography></Stack>}{error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}{role && children}</Box>;
}
