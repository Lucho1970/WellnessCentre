...
export function StaffSignIn({ children }: { children: (roles: string[]) => ReactNode }) {
  const { configured, isAuthenticated, signIn, getAccessToken } = useStaffAuth();
  ...
  return <Box>{busy && <Stack alignItems="center" py={4}><CircularProgress/><Typography mt={2}>Verifying staff access…</Typography></Stack>}{error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}{roles.length > 0 && children(roles)}</Box>;
}
