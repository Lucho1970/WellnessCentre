...
The Contact page includes an **Our Team** directory. Practitioners are shown before administration. Each grid tile contains only the approved profile image (or initials), display name and public title. Hover, keyboard focus, click and touch open the profile card. An active practitioner can expose a **Book a session** action that carries their public practitioner ID into availability browsing.
...
6. Publish it, confirm it appears on Contact, and test the profile card with mouse, keyboard and a mobile-width browser.
