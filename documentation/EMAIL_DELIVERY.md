...
The message contains the appointment date/time, a sign-in link, and a privacy-minimal `.ics` calendar attachment. Confirmation and change messages carry an event request; cancellation messages carry a cancellation with the same event ID and an increased sequence. The attachment contains no client name, service, clinical notes, or visit address. The linked client can also download an `.ics` file from **My appointments** in the client portal. Mail delivery and automatic appearance in a recipient's calendar are not guaranteed; their email/calendar provider and settings may require opening or accepting the event. This uses the existing mail permission and requires no database migration. It is bilingual until client language preference is integrated with notification delivery.
