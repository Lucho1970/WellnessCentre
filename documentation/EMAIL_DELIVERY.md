...
### Immediate appointment notices with scheduled fallback

After a new appointment, reschedule, or cancellation commits, the API now attempts delivery for that appointment immediately after writing its JSON response. SMS is attempted first, then up to three emails. The browser receives the booking result before dispatch on FastCGI/LiteSpeed; on a host without a response-finishing function, the PHP request can remain open during dispatch, although the booking has already committed. If `MAIL_ENABLED=false`, no immediate attempt runs. Existing queued items are not retroactively triggered by deploying this code.

The 15-minute Azure Logic App recurrence remains enabled as the recovery path for notices missed because the request process ended or a definite provider failure is retryable. Both paths use the same `notification_events` claim/lease and must never independently call provider APIs for a queued event. An ambiguous send outcome remains `needs_review`, not an automatic retry. No new secret, Azure workflow, frontend build, or SQL migration is required for this change; deploy the private API package and leave the existing scheduler in place. To verify, create a test booking for controlled recipients and confirm the new SMS/email events move to `sent` shortly after the booking response, before the next recurrence. Also check the generic `Wellness immediate notification` count in the PHP error log. If they stay `queued`, inspect the PHP error log for `Wellness immediate notification failed` and ensure the 15-minute run sends them; do not manually requeue `needs_review` events without checking delivery first.
...
An opt-in practitioner can also receive a separate, minimal staff appointment email at the selected work and/or verified personal address. This requires migration `018_staff_notification_preferences.sql` and the matching API/portal release. See [Staff appointment notifications](STAFF_NOTIFICATIONS.md) for the verification flow, test checklist, and SMS activation gate.
