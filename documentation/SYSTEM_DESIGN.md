...
Existing upgrade scripts are the numbered files in `api/database/migrations`, currently 001 through 014. Apply only the migrations required after the last verified deployment, in numeric order. Fresh-install `schema.sql` is not a repeatable upgrade script for an existing database. Inspect actual schema before applying any migration; do not rerun a bulk create or seed file to repair a live deployment. Migration 014 preserves the location/timezone used for time-off entries so administrators can safely view and edit them from the practitioner-first availability screen.
